...
public class GeoModelTransformer extends HumanoidModelTransformer {
	// Inflación mínima para evitar que el cuerpo traspase la armadura ajustada
	private static final float INFLATION_FACTOR = 0.005F;
	...
	// noneAttachmentArea asimétrico: frente -0.225 intacto, espalda extendida a 0.65 para capas y faldones traseros
	static final PartTransformer<GeoCube> CHEST = new ChestPartTransformer(8, 7, 1.125F, new AABB(-0.45D, 0.70D, -0.225D, 0.45D, 1.55D, 0.65D));
	...
	private static void bake(PoseStack poseStack, GeoModelPartition modelpartition, String partName, GeoBone geoBone, List<SingleGroupVertexBuilder> vertices, Map<MeshPartDefinition, IntList> indices, List<String> path, PartTransformer.IndexCounter indexCounter, boolean bindPartAnimation) {
		...
		if (!geoBone.isHidden()) {
			for (GeoCube cube : geoBone.getCubes()) {
				...
				MeshPartDefinition partDefinition = GeoMeshPartDefinition.of(partName, null, null, null, geoBone.getName());
				...
				if (bindPartAnimation) {
					...
					partDefinition = GeoMeshPartDefinition.of(partName, newList, invertedParentTransform, modelpartition.geoBone, geoBone.getName());
				}
			...
			for (GeoQuad quad : cube.quads()) {
				...
				for (GeoVertex vertex : quad.vertices()) {
					...
					vertices.add(new SingleGroupVertexBuilder()
						.setPosition(new Vec3f(pos.x() + norm.x() * INFLATION_FACTOR, pos.y() + norm.y() * INFLATION_FACTOR, pos.z() + norm.z() * INFLATION_FACTOR))
	...
	static class ChestPartTransformer extends PartTransformer<GeoCube> {
		...
		// GeckoLib vertices son block space (pixels/16), los Y deben estar divididos por 16
		static final VertexWeight[] WEIGHT_ALONG_Y = { new VertexWeight(0.85416F, 0.230F, 0.770F), new VertexWeight(0.98958F, 0.254F, 0.746F), new VertexWeight(1.125F, 0.5F, 0.5F), new VertexWeight(1.26041F, 0.744F, 0.256F), new VertexWeight(1.39583F, 0.770F, 0.230F)};
		...
		public void bakeCube(PoseStack poseStack, MeshPartDefinition partName, GeoCube cube, List<SingleGroupVertexBuilder> vertices, Map<MeshPartDefinition, IntList> indices, PartTransformer.IndexCounter indexCounter) {
			...
			// Bypass shoulder pad detection para huesos que son claramente parte del cuerpo
			// (alas, caparazón, colas) — sin esto van al joint del brazo y se estiran al atacar
			boolean skipShoulderPad = false;
			if (partName instanceof GeoMeshPartDefinition geoDef && geoDef.currentBoneName() != null) {
				String nameL = geoDef.currentBoneName().toLowerCase();
				if (nameL.contains("wing") || nameL.contains("shell") || nameL.contains("tail") || nameL.contains("fin")) {
					skipShoulderPad = true;
				}
			}

			// Hombreras: Y sin modificar, X=0.25 y Z=0.25 en block space para capturar relieves
			if (!skipShoulderPad && centerOfCube.y >= this.yClipCoord && Math.abs(centerOfCube.x) > 0.25D && centerOfCube.z < 0.25D) {
			...
			// Asignación directa de pesos por vértice — sin recorte de polígonos.
			// El sistema de clipping original asume quads axis-aligned (modelos vanilla).
			// Los modelos GeckoLib pueden tener quads rotados → división por cero → vértices al infinito.
			for (GeoQuad polygon : cube.quads()) {
				...
				for (GeoVertex vertex : polygon.vertices()) {
					Vector4f pos = new Vector4f(vertex.position(), 1.0F);
					pos.mul(matrix);
					
					VertexWeight w = getYClipWeight(pos.y());
					float weight1 = w.chestWeight;
					float weight2 = w.torsoWeight;
					int joint1 = 8;
					int joint2 = 7;
	...
	public record GeoMeshPartDefinition(String partName, List<String> path, OpenMatrix4f invertedParentTransform, GeoBone root, String currentBoneName) implements MeshPartDefinition {
		public static MeshPartDefinition of(String partName) {
			return new GeoMeshPartDefinition(partName, null, null, null, null);
		}
		
		public static MeshPartDefinition of(String partName, List<String> path, OpenMatrix4f invertedParentTransform, GeoBone root) {
			return new GeoMeshPartDefinition(partName, path, invertedParentTransform, root, null);
		}
		
		public static MeshPartDefinition of(String partName, List<String> path, OpenMatrix4f invertedParentTransform, GeoBone root, String currentBoneName) {
			return new GeoMeshPartDefinition(partName, path, invertedParentTransform, root, currentBoneName);
		}
